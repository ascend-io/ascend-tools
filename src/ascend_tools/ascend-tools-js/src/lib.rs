...
impl Client {
    ...
    pub fn otto_chat_streaming(&self, prompt: String, on_delta: ThreadsafeFunction<String, UnknownReturnValue>, workspace: Option<String>, deployment: Option<String>, uuid: Option<String>, thread_id: Option<String>, model: Option<String>, provider: Option<String>) -> AsyncTask<TypedTask<models::OttoChatResponse>> {
        ...
        AsyncTask::new(TypedTask::new(move || {
            ...
            let runtime_id = client.resolve_optional_runtime_target(workspace.as_deref(), deployment.as_deref(), uuid.as_deref()).map_err(to_napi_err)?;
            let request = models::OttoChatRequest { prompt, runtime_id, thread_id, model: otto_model };
            client.otto_chat_streaming(&request, |delta| { on_delta.call(Ok(delta.to_string()), ThreadsafeFunctionCallMode::NonBlocking); }).map_err(to_napi_err)
        }))
    }
